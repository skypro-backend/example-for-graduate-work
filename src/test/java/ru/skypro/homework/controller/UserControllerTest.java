...
class UserControllerTest {
    ...
    void testSetUserPassword() {
        // Мокаем userService.setPassword, чтобы он ничего не делал
        doNothing().when(userService).setPassword(any());

        // Вызываем метод контроллера
        ResponseEntity<?> responseEntity = userController.setUserPassword(new NewPasswordDTO("oldPassword", "newPassword"));

        // Проверяем, что возвращается ожидаемый ResponseEntity
        assertEquals(200, responseEntity.getStatusCodeValue());

        // Проверяем, что userService.setPassword вызывается с правильным параметром
