...
create table users
(
id BIGINT not null primary key,-- Уникальный идентификатор пользователя
...
(
    ...
    FOREIGN KEY (pk) REFERENCES ad(pk) -- Предполагается, что 'pk' в 'comments' является внешним ключом, ссылаясь на 'pk' в 'ad'
);
